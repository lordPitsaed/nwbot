...
export const getRndGeogssrImage = async () => {
  const getHintRegExp = (key: number) =>
    new RegExp(
      '<div id="info-box-gallery' +
        key +
        '" class="info-box-gallery">(?<hint>.+)<\\/div><\\/div>',
      "gim"
    );
  const imageIdRegExp =
  ...
  const fetchMarkup = async (offset: number) =>
    fetch(`https://www.mapcrunch.com/gallery?offset=${offset}`).then((resp) =>
      resp.text()
    );
  const fetchMapData = async (id: string) =>
    fetch(`http://www.mapcrunch.com/s/${id}`).then((resp) => resp.text());

  const getLocation = async (lat: string, lon: string) =>
    fetch(
  ...
  const maxPage = +[
    ...(await fetchMarkup(999999999)).matchAll(/\<a class="static"\>\d+/gm),
  ][0]?.[0].replace(/\D/gm, "");
  ...
  if (!Number.isNaN(maxPage)) {
    ...
    const imageIdsArray = [...randomPage.matchAll(imageIdRegExp)].map(
      (match) => match.groups?.imageId
    );

    const randomImage =
      imageIdsArray[await getRandInt(0, imageIdsArray.length - 1, true)];

    const mapData = [
      ...(await fetchMapData(randomImage || "")).matchAll(coordsRegExp),
    ]
