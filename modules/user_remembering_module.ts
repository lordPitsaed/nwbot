  ...
  bot.onText(/\/rememberme/, (msg) => {
    if (msg.chat.type === "private") {
      if (!userIds.includes(String(msg.from?.id))) {
        ...
        fs.writeFileSync(
          envVars.REMEMBERED_USER_IDS,
          JSON.stringify({ userIds: newArr })
        );
        bot.sendMessage(
          msg.chat.id,
          `Q ${msg.from?.username}, ur id is: ${msg.from?.id}. I'll remember you`
        );
        getUserNamesById(bot, userIds, envVars.CHAT_ID);
      } else {
        bot.sendMessage(
          msg.chat.id,
          "I can remember you already, " + msg.from?.username
        );
      }
    } else {
