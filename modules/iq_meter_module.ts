...
const IQ = './data_files/iq.txt';
const IQ_PBS = './data_files/iq_pbs.txt';


export const iqMeterModule = () => {
  try {
    fs.readFileSync(IQ);
    fs.readFileSync(IQ_PBS);
  } catch {
    fs.writeFileSync(IQ, '{}');
    fs.writeFileSync(IQ_PBS, '{}');
  }

  let iqs: TIqMap = JSON.parse(fs.readFileSync(IQ, { encoding: 'utf-8' }));
  let iqsPbs: TIqMap = JSON.parse(fs.readFileSync(IQ_PBS, { encoding: 'utf-8' }));
  ...
  bot.on('text', async (msg) => {
    if (msg.text?.toLowerCase() === 'айку' || msg.text?.toLowerCase() === 'iq') {
      ...
      fs.writeFileSync(IQ, JSON.stringify(iqs));
      fs.writeFileSync(IQ_PBS, JSON.stringify(iqsPbs));
  ...
  schedule.scheduleJob(rule, () => {
    fs.writeFileSync(IQ, '{}');
    iqs = JSON.parse(fs.readFileSync(IQ, { encoding: 'utf-8' }));
